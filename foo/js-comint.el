...
\(Type \\[describe-mode] in the process buffer for a list of commands.)"
  ...
  (if (not (comint-check-proc "*nodejs*"))
      (save-excursion (let ((cmdlist (split-string cmd)))
	(set-buffer (apply 'make-comint "nodejs" (car cmdlist)
			   nil (cdr cmdlist)))
	(inferior-js-mode))))
  (setq inferior-js-program-command cmd)
  (setq inferior-js-buffer "*nodejs*")
  (if (not dont-switch-p)
      (pop-to-buffer "*nodejs*")))
...
(defun js-send-region (start end)
  ...
  (if (not (char-equal (char-before end) ?\n))
...
;;;###autoload
(defun js-send-current-line ()
  "Send the current line to the inferior Javascript process."
  (interactive)
  (js-send-region (line-beginning-position) (line-end-position)))
